...
export function v1Router() {
  ...
  r.post("/curator/scan", async (c) => {
    ...
    const result = await curatorService(c.get("sql"), c.get("firecrawl"), c.get("typesafe")).scan(parsed.data);
  ...
  r.post("/curator/scrape", async (c) => {
    ...
    if (!parsed.success) throw zodTo422(parsed.error.issues);
    const result = await curatorService(c.get("sql"), c.get("firecrawl"), c.get("typesafe")).scrape(parsed.data.urls);
    return c.json(result);
  });
  ...
  r.post("/curator/issues", async (c) => {
    ...
    const created = await curatorService(c.get("sql"), c.get("firecrawl"), c.get("typesafe")).publish({
      slug: body.slug,
