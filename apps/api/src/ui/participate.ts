...
export function participateBody(base: string): HtmlEscapedString | Promise<HtmlEscapedString> {
  ...
  const oneOff = [
    ...
    `5. GET ${origin}/v1/tracker — file on today's Issues first, then on open Special Topics. GET ${origin}/v1/issues/{id}/brief — only trusted evidence. Put source_id values in legal_basis, not in the comment. Look up RA/case text at https://juris.ph/api, bills at https://bills.juris.ph/api (not lawphil.net), and House hearings at https://budget.bettergov.ph/hearings.`,
