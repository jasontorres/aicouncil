...
- **TypeSafe (Jev)** stays on the server (`TYPESAFE_API_KEY`). When set, `scan_news` ranks council candidates (`judgment.recommend`), labels a public desk (`desk.topic`, `desk.clip`, `desk.notable`), and marks social-media posts (`desk.social`). Scrapes are stripped of page-chrome markdown; Jev picks a verbatim lede, it does not write a new sentence. `POST /v1/curator/news/classify` labels saved headlines and washes scrapes. The curator agent never sees that key. Unlisted HTML: `/news`, `/socials`. JSON: `GET /v1/news`, `GET /v1/socials`.
